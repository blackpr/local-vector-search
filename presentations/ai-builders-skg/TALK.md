...
- Browser: Latent open, green **System Ready**, the 33 notes from [talk-brain.json](talk-brain.json) imported, zoom 150%.
...
2. **EmbeddingGemma.** Google, on-device, multilingual, which you just saw. The prefixes are the gotcha. The tagging prompt gets a laugh: "peak prompt engineering".
...
2. **One word cost 1.3 GB.** The code said `dtype: 'fp32'`, with a comment next to it: "SQLite expects float32". Sounds reasonable. SQLite does want float32 vectors. But `dtype` isn't about the vectors that come out, it's about the precision of the model file you download, and the output is float32 no matter what. Switching that one word to `q4`: about 1.6 GB became 317 MB, and every question in my test set still returns the same note. *Measure before you assume quality needs the big file.*
...
2. Fresh Chrome profile. Wait for **System Ready** (first time downloads about 300 MB).
