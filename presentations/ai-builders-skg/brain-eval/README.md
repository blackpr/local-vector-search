...
Last run (19 Sep 2026, Node, CPU): 40 of 41 queries returned the expected note on top (q4). An earlier 32-query version gave identical results with fp32, q8 and q4. The last one is `the weather in london tomorrow`, which is supposed to return nothing (top distance 1.07, above the cutoff).
